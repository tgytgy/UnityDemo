...
public class CheckUpdateAndDownload : MonoBehaviour
{
    ...
    IEnumerator DoUpdateAddressadble()
    {
        ...
        if (checkHandle.Result.Count > 0)
        {
            ...
            // 更新列表迭代器
            //每个 IResourceLocator 通常对应一个资源目录（catalog）
            List<IResourceLocator> locators = updateHandle.Result;
            foreach (var locator in locators)
            {
                List<object> keys = new List<object>();
                //资源键(Keys) 每个 IResourceLocator 包含该 Catalog 中所有资源的键（Keys），这些键是用来唯一标识和加载资源的
                keys.AddRange(locator.Keys);
                // 获取待下载的文件总大小
                using var enumerator = keys.GetEnumerator();
                var sizeHandle = Addressables.GetDownloadSizeAsync(enumerator);
